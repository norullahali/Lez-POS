...
class PurchasesDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<List<PurchaseInvoice>> getAllInvoices() => (select(purchaseInvoices)
        ..orderBy([(i) => OrderingTerm.desc(i.purchaseDate)]))
      .get();

  Stream<List<PurchaseInvoice>> watchAllInvoices() => (select(purchaseInvoices)
        ..orderBy([(i) => OrderingTerm.desc(i.purchaseDate)]))
      .watch();
  ...
  Future<List<PurchaseItem>> getItemsForInvoice(int invoiceId) =>
      (select(purchaseItems)..where((item) => item.invoiceId.equals(invoiceId)))
          .get();

  Future<PurchaseItem?> getPurchaseItemById(int id) =>
      (select(purchaseItems)..where((item) => item.id.equals(id)))
          .getSingleOrNull();
  ...
  Future<List<Map<String, dynamic>>> getPurchasesBySupplier(
    ...
    final rows = await customSelect(
      ...
      readsFrom: {
        purchaseInvoices,
        db.suppliers
      }, // Need to refer to suppliers table on DB level
    ).get();
