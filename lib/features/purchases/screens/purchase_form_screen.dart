...
class _PurchaseFormScreenState extends ConsumerState<PurchaseFormScreen> {
  ...
  void initState() {
    super.initState();
    // No listener needed — ValueListenableBuilder handles dropdown reactivity.
  }
  ...
  Widget build(BuildContext context) {
    ...
    final supplierBalanceAsync = formState.supplierId != null
        ? ref.watch(supplierBalanceProvider(formState.supplierId!))
                                      ...
                                      child: InputDecorator(
                                        decoration: const InputDecoration(
                                            ...
                                            suffixIcon: Icon(
                                                ...
                                                size: 18)),
                                        child: Text(formState.dueDate != null
                                    ...
                                    child: Container(
                                      ...
                                      child: Row(
                                        children: [
                                          ...
                                          Text(
                                              'رصيد الديون السابقة: ${nf.format(supplierBalanceAsync.valueOrNull)} د.ع',
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                // ValueListenableBuilder rebuilds the dropdown
                                // on every keystroke; the Consumer inside still
                                // holds the product-list data from the provider.
                                ValueListenableBuilder<TextEditingValue>(
                                  ...
                                  builder: (context, textValue, _) {
                                    ...
                                    return Consumer(builder: (ctx, r, _) {
                                      final productsAsync =
                                          r.watch(productsNotifierProvider);
                              ...
                              children: [
                                ...
                                if (formState.items.isNotEmpty) ...[
                                  ...
                                  _TotalRow(
                                      ...
                                      value:
                                          '${nf.format(formState.total - formState.paidAmount)} د.ع',
                                      bold: true,
                                      valueColor: (formState.total -
  ...
  Future<void> _addByBarcode(String barcode) async {
    ...
    // Guard against re-entrant calls from fast scanners / double-Enter.
    if (_isBarcodeProcessing) {
      debugPrint('[Purchase] Barcode ignored — already processing a scan.');
      return;
    }

    // Normalize Arabic/Persian digits: '٤٥٦٤٥٧' → '456457'
    barcode = barcode.normalizeBarcode();
    ...
    try {
      ...
      // Clear field and restore focus only AFTER the dialog fully closes.
      if (mounted) {
        _barcodeCtrl.clear();
  ...
  Future<void> _showAddItemDialog(int productId, String productName,
    ...
    try {
          ...
          content: SizedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                    controller: discCtrl,
                    decoration: const InputDecoration(labelText: 'خصم البند'),
                    keyboardType:
                        const TextInputType.numberWithOptions(decimal: true)),
              ],
            ),
      ...
      if (result == true && mounted) {
        ...
        final cost = double.tryParse(_normalizeNumber(costCtrl.text.trim())) ??
            defaultCost;
        final disc =
